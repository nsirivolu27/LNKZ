...
**How to apply:** Keep the managed Expo workflow, use the Expo-compatible host mode for device access, and make any static bundle builder honor an explicit alternate Metro port when another workflow occupies 8081. If both external Replit domains return `Invalid Host` while local Metro is healthy, this is an edge-routing issue; tunnel mode requires the optional `@expo/ngrok` package and may fail independently. A successful static Expo bundle and app-shell HTTP response can validate the artifact when the browser screenshot endpoint returns an HTTP failure for the live Expo session.