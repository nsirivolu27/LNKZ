...
import type { Actor, Connector, ContextItem, ContextPacket, Conversation } from "../types.js";
...
export async function buildContextPacket(
  ...
  const selected = await selectConversations(store, request, maxConversations, actor);
...
async function selectConversations(
  ...
  if (request.query && selected.length < maxConversations) {
    for (const match of await store.search(request.query, maxConversations, actor)) {
      if (seen.has(match.id)) continue;
      const conversation = await store.get(match.id, actor);
  ...
  if (!selected.length && !request.query) {
    for (const summary of await store.list({ limit: maxConversations }, actor)) {
