...
import type { Actor, Connector, Conversation, ConversationInput } from "./types.js";
...
export function createLnkzMcpServer(
  ...
  const scopedConnectors = connectors.map((connector) =>
    connector.id === "lnkz" ? createLnkzConnector(store, actor) : connector);
  ...
  server.registerTool(
    ...
    async (input) => {
      const conversation = await store.save(conversationInputSchema.parse(input) as ConversationInput, actor);
      return ok(
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      const conversation = await store.get(id, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const conversations = await store.list(options, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const conversation = await store.appendMessages(conversationId, messages, actor);
      if (!conversation) return toolError("Conversation not found.");
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      const removed = await store.remove(id, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      try {
        const handoff = await store.createHandoff(options, actor);
        return ok(
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const packet = await store.redeemHandoff(token, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const packet = await store.redeemHandoff(options.token, actor);
      if (!packet) return toolError("Handoff is invalid, revoked, exhausted, or expired.");
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const revoked = await store.revokeHandoff(handoffId, actor);
  ...
  server.registerTool(
    ...
    async ({ conversationId }) => {
      const handoffs = await store.listHandoffs(conversationId, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const packet = await buildContextPacket(store, scopedConnectors, request, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const conversation = await store.get(conversationId, actor);
      if (!conversation) return toolError("Conversation not found.");
      const analysis = analyzeConversation(conversation);
      const lines = [
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const conversations = await loadRecent(store, limit, actor);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const result = await aggregateSearch(scopedConnectors, request);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      const events = await store.listEvents(limit, actor);
  ...
  server.registerResource(
    ...
    async () => jsonResource("lnkz://stats", await store.stats(actor)),
  );
  ...
  server.registerResource(
    ...
    async () => jsonResource("lnkz://conversations", { conversations: await store.list({ limit: 25 }, actor) }),
  );
  ...
  server.registerResource(
    ...
    async (uri, variables) => {
      ...
      const conversation = id ? await store.get(id, actor) : null;
      if (!conversation) {
...
async function loadRecent(store: ConversationStore, limit: number, actor: Actor): Promise<Conversation[]> {
