import { client, type Analysis, type ConversationSummary, type HandoffSummary } from "./api";
...
<section class="card auth-card">
  ...
  <p class="hint">Your session stays in an HttpOnly cookie. LNKZ never stores your password in the browser.</p>
  <form id="auth-form">
    <label>Email <input id="auth-email" type="email" autocomplete="email" required /></label>
    <label>Password <input id="auth-password" type="password" autocomplete="current-password" minlength="12" required /></label>
    <div class="actions"><button class="button primary" type="submit">Sign in</button><button id="auth-signup" class="button secondary" type="button">Create account</button></div>
    <p id="auth-result" class="result" hidden></p>
  </form>
</section>`;
  ...
  document.getElementById("auth-signup")!.addEventListener("click", () => void submit(true));
}
