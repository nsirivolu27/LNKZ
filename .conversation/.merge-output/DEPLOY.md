...
| --- | --- |
| `LNKZ_API_KEY` | Optional legacy compatibility credential. Prefer account login and scoped API tokens; the server logs when this fallback is enabled. |
| `LNKZ_REQUIRE_HTTPS` | Set `true` to reject cleartext requests with `426 Upgrade Required`; production should always use HTTPS. |
| `TRUST_PROXY` | Set `true` only when a trusted reverse proxy sets `X-Forwarded-Proto`; this enables correct HTTPS detection behind the proxy. |
| `LNKZ_COOKIE_SECURE` | Defaults to `true`. Set `false` only for local HTTP development; never use it on an exposed deployment. |
...
Twenty checks should pass, covering account signup/login, session auth, scoped-token creation,
cookie-only MCP rejection, import, search, packet building, handoff
...
On first boot of a v1 database, LNKZ copies the live file to
`lnkz.db.v1-backup-<timestamp>` before applying the v2 schema and rebuilding FTS5. Stop the
process before restoring a backup, preserve the migrated file for forensics, then replace
`lnkz.db` with the timestamped backup and start the previous application version. Do not
restore a v1 file over a running WAL database; remove or preserve its `-wal` and `-shm` files
as a set according to your SQLite backup procedure.
