...
let apiKey = process.env.LNKZ_API_KEY ?? randomBytes(12).toString("hex");
const accountEmail = `smoke-${randomBytes(6).toString("hex")}@example.com`;
...
async function main() {
  ...
  const signup = await publicRequest("/api/auth/signup", {
    method: "POST",
    body: JSON.stringify({ email: accountEmail, password: accountPassword }),
  });
  check("signup creates an account without exposing enumeration", signup.status === 202 && signup.body?.ok === true);

  const login = await publicRequest("/api/auth/login", {
    method: "POST",
    body: JSON.stringify({ email: accountEmail, password: accountPassword }),
  });
  sessionCookie = login.headers.get("set-cookie")?.split(";")[0];
  check("login establishes an HttpOnly session", login.status === 200 && Boolean(sessionCookie) && login.body?.ok === true);

  const session = await publicRequest("/api/session", { headers: { cookie: sessionCookie } });
  check("the session resolves the current workspace actor", session.status === 200 && session.body?.authenticated === true);

  const cookieMcp = await fetch(`${baseUrl}/mcp`, {
    method: "POST",
    headers: { cookie: sessionCookie, "content-type": "application/json", accept: "application/json" },
    body: JSON.stringify({ jsonrpc: "2.0", id: 0, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "smoke", version: "1" } } }),
  });
  check("MCP rejects cookie-only authentication", cookieMcp.status === 401);

  const tokenIssue = await publicRequest("/api/tokens", {
    method: "POST",
    headers: { cookie: sessionCookie },
    body: JSON.stringify({ scope: "admin" }),
  });
  apiKey = tokenIssue.body?.token;
  check("owner can mint a scoped API token", tokenIssue.status === 201 && typeof apiKey === "string" && apiKey.length > 20);
