...
LNKZ is self-hosted by design. There is no LNKZ cloud and nothing phones home. When the
console is exposed beyond a single local user, accounts, workspaces, and server-side sessions
keep access scoped.
...
- Redemptions are `no-store` and `noindex`, and the public share route is rate limited per IP
  and per handoff token
...
<<<<<<< .merge-input/local/README.md
Working MVP, self-hosted. Accounts, workspaces, owner/member/viewer roles, scoped API tokens,
server-side sessions, workspace-isolated SQLite queries, security headers, HTTPS enforcement,
and migration backups are shipped. The test suite and end-to-end smoke test boot the built
server and drive it over both REST and MCP.
=======
...
>>>>>>> .merge-input/current/README.md

```bash
npm test && npm run typecheck && npm run build && node scripts/smoke.mjs
```

**Explicitly deferred:** password reset and email verification, 2FA, SSO/OIDC, conversation
content encryption, shared multi-instance rate limiting, semantic search, OAuth for connectors,
and writing back to connected systems. See [ROADMAP.md](ROADMAP.md).
