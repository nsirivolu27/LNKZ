...
export interface Session {
  ...
  actor?: { id: string; workspaceId: string; role: "owner" | "member" | "viewer" };
  expiresAt?: string;
}
...
export const client = {
  ...
  login: (email: string, password: string) =>
    api<{ ok: true }>("/api/auth/login", { method: "POST", body: JSON.stringify({ email, password }) }),
  signup: (email: string, password: string) =>
    api<{ ok: true }>("/api/auth/signup", { method: "POST", body: JSON.stringify({ email, password }) }),
