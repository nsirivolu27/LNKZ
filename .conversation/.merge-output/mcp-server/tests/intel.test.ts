...
import type { Actor, Conversation } from "../src/types.js";
...
test("a context packet respects its token budget and reports what it used", async (t) => {
  ...
  const packet = await buildContextPacket(store, [], { query: "rollout", budgetTokens: 900, includeExternal: false }, actor);
...
test("a packet built from explicit ids does not need a query", async (t) => {
  ...
  const packet = await buildContextPacket(store, [], { conversationIds: [saved.id], includeExternal: false }, actor);
