...
const testActor: Actor = { id: "test-user", workspaceId: "default", role: "owner", auth: "session", scope: "admin" };
...
async function connect(t: { after: (fn: () => unknown) => void }) {
  ...
  const server = createLnkzMcpServer(store, [createLnkzConnector(store)], "https://lnkz.example", testActor);
...
test("import runs as a dry run before it writes anything", async (t) => {
  ...
  assert.equal((await store.list({}, testActor)).length, 0, "a dry run must not write");
  ...
  assert.ok((await store.get(imported[0].id, testActor))?.tags.includes("migrated"));
});
