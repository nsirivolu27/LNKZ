...
import { LnkzApiError, LnkzClient, resolveProfile, verifyExpectedWorkspace } from "../client.js";
...
test("named profiles select apiKeyEnv without exposing its value", () => {
  const profile = resolveProfile({
    LNKZ_PROFILE: "team",
    LNKZ_PROFILES_JSON: JSON.stringify([
      { name: "personal", baseUrl: "https://personal.example", apiKeyEnv: "PERSONAL_KEY", workspaceId: "personal-id" },
      { name: "team", baseUrl: "https://team.example", apiKeyEnv: "TEAM_KEY", workspaceId: "team-id" },
    ]),
    TEAM_KEY: "do-not-print",
  });
  assert.deepEqual({ name: profile.name, baseUrl: profile.baseUrl, apiKeyEnv: profile.apiKeyEnv, workspaceId: profile.workspaceId }, {
    name: "team", baseUrl: "https://team.example", apiKeyEnv: "TEAM_KEY", workspaceId: "team-id",
  });
  assert.throws(() => resolveProfile({
    LNKZ_PROFILES_JSON: JSON.stringify([{ name: "team", baseUrl: "https://team.example", apiKeyEnv: "TEAM_KEY", workspaceId: "team-id" }]),
  }), /Missing API key/);
});

test("workspace verification fails closed on a profile mismatch", async () => {
  const client = { getWorkspace: async () => ({
    workspace: { id: "actual", name: "Workspace", mode: "personal", useCase: "test", datasets: { enabled: false } },
    access: { actorId: "actor", scopes: [] },
  }) };
  await assert.rejects(verifyExpectedWorkspace(client, "expected"), /does not match/);
});
