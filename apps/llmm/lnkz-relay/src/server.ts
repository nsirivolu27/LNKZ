...
import { LNKZ_VERSION } from "./version.js";
import {
...
if (existsSync(webDist)) {
  app.use(express.static(webDist));
    app.get(/^\/(?!api|share|health).*/, (_request, response) => {
...
const httpServer = app.listen(port, host, (error?: Error) => {
  ...
  if (!process.env.LNKZ_API_KEY?.trim()) {
    console.warn("[server] LNKZ_API_KEY is not set: the REST API is unauthenticated outside production.");
  }
});
