...
import { verifyContext } from "./context.js";
...
export function requireApiKey(request: Request, response: Response, next: NextFunction): void {
  ...
  const context = verifyContext(
    ...
    process.env.LNKZ_MCP_CONTEXT_SECRET?.trim(),
  );
  if (context) {
    // A valid managed context is accepted only after its signature, expiry, and
    // required identity fields have been checked. Plain workspace headers are ignored.
    next();
    return;
  }
  response.status(401).json({ error: "Unauthorized" });
